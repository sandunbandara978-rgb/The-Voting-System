...
const DB_FILE_PATH = process.env.VERCEL || process.env.NODE_ENV === 'production'
    ? path_1.default.join('/tmp', 'election_store.json')
    : path_1.default.join(__dirname, 'election_store.json');
...
class ElectionStore {
    ...
    saveToDisk(state) {
        try {
            ...
            fs_1.default.writeFileSync(DB_FILE_PATH, JSON.stringify(toSave, null, 2), 'utf-8');
    ...
    getCandidates(districtId, partyId) {
        return this.data.candidates.filter(c => {
            if (districtId && c.districtId !== districtId)
                return false;
            if (partyId && c.partyId !== partyId)
                return false;
            return c.isActive;
        });
    }
    // --- Voter Identity & Verification ---
    verifyVoterIdentity(type, docNumber, dob) {
        const cleanDoc = docNumber.trim().toUpperCase();
        let voter = this.data.voters.find(v => v.identityType === type && v.documentNumber.toUpperCase() === cleanDoc);
        if (!voter) {
    ...
    castBallot(voterId, districtId, partyId, candidateIds) {
        ...
        const hash = crypto_1.default.createHash('sha256').update(`${election.id}-${districtId}-${timestamp}-${salt}`).digest('hex');
        ...
        const ballot = {
            id: `ballot-${Date.now()}-${crypto_1.default.randomBytes(4).toString('hex')}`,
...
exports.dbStore = new ElectionStore();
